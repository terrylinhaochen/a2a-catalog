...
-- Insert additional agents from the A2A Directory (avoiding duplicates)
INSERT INTO public.agents (name, description, provider, categories, skills, votes, is_verified, github_url, documentation, featured) 
SELECT * FROM (VALUES
...
) AS new_agents(name, description, provider, categories, skills, votes, is_verified, github_url, documentation, featured)
WHERE NOT EXISTS (
  SELECT 1 FROM public.agents 
  WHERE agents.name = new_agents.name 
  AND agents.provider = new_agents.provider
);
